...
class ActionWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        for (action in actionList) {
            try {
                // 本版本只保留“短信/消息 -> Gotify”转发。任何自动任务动作都不得
                // 修改设置、发送短信、启动服务、执行 Frpc/WOL 或触发其他外联。
                if (action.type != TASK_ACTION_NOTIFICATION) {
                    writeLog("已忽略非 Gotify 转发动作: ${action.type}", "WARN")
                    continue
                }
                when (action.type) {
                    TASK_ACTION_SENDSMS -> {
                    ...
                    TASK_ACTION_HTTPSERVER -> {
                        if (!App.HTTP_SERVER_FEATURE_ENABLED) {
                            writeLog("HTTP Server 远程控制功能已禁用")
                            continue
                        }
                    ...
                    TASK_ACTION_WOL -> {
                        // WOL 会发送网络数据包，与本版本“仅 Gotify 转发”目标无关。
                        writeLog("WOL 功能已禁用")
                        continue
                    }
