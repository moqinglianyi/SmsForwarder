...
class SmsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        try {
            ...
            if (intent.action == Telephony.Sms.Intents.WAP_PUSH_RECEIVED_ACTION || intent.action == Telephony.Sms.Intents.WAP_PUSH_DELIVER_ACTION) {
                ...
                Log.d(TAG, "收到短信，来源已脱敏，长度=${msg.length}")
            } else {
            ...
            //短信指令
            if (App.SMS_COMMAND_FEATURE_ENABLED && SettingUtils.enableSmsCommand && msg.startsWith("smsf#")) {
            ...
            Log.d(TAG, "已入队短信转发任务，simSlot=$simSlot")
    ...
    private fun doSmsCommand(context: Context, from: String, message: String) {
        ...
        Log.d(TAG, "已加载短信指令白名单")

        // 白名单为空时不得执行任何远程控制指令。
        if (TextUtils.isEmpty(safePhone)) {
            Log.w(TAG, "短信指令已拒绝：未配置安全手机号")
            return
        }
