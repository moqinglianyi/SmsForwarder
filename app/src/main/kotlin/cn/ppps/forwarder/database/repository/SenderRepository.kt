...
class SenderRepository(private val senderDao: SenderDao) {
    ...
    fun insert(sender: Sender) {
        if (sender.type == TYPE_GOTIFY) senderDao.insert(sender)
    }
    ...
    fun update(sender: Sender) {
        if (sender.type == TYPE_GOTIFY) senderDao.update(sender.id, sender.name, sender.jsonSetting, sender.status)
    }
    ...
    fun getAllNonCache(): List<Sender> {
        val query = SimpleSQLiteQuery("SELECT * FROM Sender WHERE type = 11 ORDER BY id ASC")
