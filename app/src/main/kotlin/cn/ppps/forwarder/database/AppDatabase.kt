            ...
            val builder = Room.databaseBuilder(
                ...
                .addCallback(object : Callback() {
                    override fun onCreate(db: SupportSQLiteDatabase) {
                        ...
                        // 不再预置远程 Frpc 配置，避免安装后形成外连/远程控制通道。
                    }
                }).addMigrations(
                    MIGRATION_1_2,
                    MIGRATION_2_3,
...
""".trimIndent()
        ...
        // 删除旧版本可能已写入的默认 Frpc 远程控制配置。
        private val MIGRATION_21_22 = object : Migration(21, 22) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("DELETE FROM Frpc WHERE uid = '830b0a0e-c2b3-4f95-b3c9-55db12923d2e'")
                // 历史版本可能保存了邮箱、Webhook、Telegram、企业微信等第三方凭据。
                // 这些通道已不再支持，升级时直接删除，避免遗留配置被误用或泄露。
                database.execSQL("DELETE FROM Sender WHERE type != 11")
            }
        }
