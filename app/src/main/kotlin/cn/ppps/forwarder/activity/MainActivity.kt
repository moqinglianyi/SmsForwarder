...
class MainActivity : BaseActivity<ActivityMainBinding?>(), DrawerAdapter.OnItemSelectedListener {
    ...
    private fun initData() {
        ...
        if (App.REMOTE_UPDATE_FEATURE_ENABLED && SettingUtils.autoCheckUpdate && NetworkUtils.isHaveInternet()) {
            showTips(this)
    ...
    override fun onItemSelected(position: Int) {
        ...
        when (position) {
            ...
            POS_SERVER, POS_CLIENT -> {
                XToastUtils.info("当前版本已关闭远程控制，仅支持 Gotify HTTPS 转发")
                return
            }
            POS_FRPC -> {
                if (!App.FRPC_FEATURE_ENABLED) {
                    XToastUtils.info("当前版本已关闭 Frpc，仅支持 Gotify 短信转发")
                    return
                }
    ...
    private fun downloadFrpcLib() {
        if (!App.FRPC_FEATURE_ENABLED) return
        ...
        XHttp.downLoad(downloadUrl).savePath(cacheDir.absolutePath)
