...
class App : Application(), CactusCallback, Configuration.Provider by Core {
    ...
    companion object {
        ...
        // 本版本仅保留 Gotify 等短信转发，关闭所有远程控制面。
        const val FRPC_FEATURE_ENABLED = false
        const val HTTP_SERVER_FEATURE_ENABLED = false
        // 短信指令可执行本地高风险操作，本版本完全关闭。
        const val SMS_COMMAND_FEATURE_ENABLED = false
        // 自动转发只允许 Gotify，避免历史配置把短信发送到其他第三方。
        const val ONLY_GOTIFY_FORWARDING = true
        // 本版本不启用统计上报和远程更新，避免设备信息/配置被第三方 SDK 采集。
        const val ANALYTICS_FEATURE_ENABLED = false
        const val REMOTE_UPDATE_FEATURE_ENABLED = false
    ...
    override fun onCreate() {
        ...
        // 不把异常对象、请求 URL、响应体或短信内容写入 logcat/缓存文件。
        // 仍交给系统默认处理器结束进程，避免在本地持久化敏感诊断信息。
        ...
        try {
            ...
            // Frpc 已禁用：不加载、不执行、不接受动态 native 库。
            if (FRPC_FEATURE_ENABLED) {
            ...
            //启动HttpServer
            if (HTTP_SERVER_FEATURE_ENABLED && HttpServerUtils.enableServerAutorun) {
    ...
    private fun initLibs() {
        ...
        // 本版本关闭远程更新和第三方统计，避免额外外联。
        if (REMOTE_UPDATE_FEATURE_ENABLED) {
            XUpdateInit.init(this)
        }
        if (ANALYTICS_FEATURE_ENABLED) {
            UMengInit.init(this)
        }
        // 初始化语种切换框架
