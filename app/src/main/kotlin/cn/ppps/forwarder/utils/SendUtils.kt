...
object SendUtils {
    ...
    fun sendMsgSender(msgInfo: MsgInfo, rule: Rule, senderIndex: Int = 0, logId: Long = 0L, msgId: Long = 0L) {
        try {
            ...
            if (sender.status != 1) {
                Log.d(TAG, "sender disabled: type=${sender.type}")
                updateLogs(logId, 0, getString(R.string.sender_disabled))
                senderLogic(0, msgInfo, rule, senderIndex, msgId)
                return
            }

            // 本版本只允许 Gotify 自动转发，历史规则中的其他通道不得发送短信。
            if (App.ONLY_GOTIFY_FORWARDING && sender.type != TYPE_GOTIFY) {
                Log.w(TAG, "已拒绝非 Gotify 转发通道: type=${sender.type}")
                updateLogs(logId, 0, "仅允许 Gotify 转发")
                senderLogic(0, msgInfo, rule, senderIndex, msgId)
                return
            }
