...
class SharedPreference<T>(private val name: String, private val default: T) : ReadWriteProperty<Any?, T> {
    ...
    companion object {
        ...
        private fun <T> deSerialization(str: String): T {
            require(str.length <= 2 * 1024 * 1024) { "配置数据过大" }
            val redStr = java.net.URLDecoder.decode(str, "UTF-8")
            val byteArrayInputStream = ByteArrayInputStream(
                redStr.toByteArray(charset("ISO-8859-1"))
            )
            val objectInputStream = object : ObjectInputStream(byteArrayInputStream) {
                override fun resolveClass(desc: java.io.ObjectStreamClass): Class<*> {
                    val name = desc.name
                    val allowed = name.startsWith("cn.ppps.forwarder.") ||
                        name.startsWith("java.lang.") ||
                        name.startsWith("java.util.") ||
                        name.startsWith("java.math.") ||
                        name == "[B" || name == "[I" || name == "[J" ||
                        name == "[F" || name == "[D" || name == "[Z" ||
                        name == "[C" || name == "[S" || name == "[Ljava.lang.String;"
                    require(allowed) { "不允许反序列化类型: $name" }
                    return super.resolveClass(desc)
                }
            }
            val obj = objectInputStream.readObject()
            require(obj is Map<*, *>) { "配置数据格式无效" }
