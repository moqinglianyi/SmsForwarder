...
class HttpServerUtils private constructor() {
    ...
    companion object {
        ...
        /**
         * HTTP Server 只允许在明确配置了安全协议后启动。
         * 0 表示无保护，禁止用于服务启动；1/2/3 分别对应签名/RSA/SM4。
         */
        fun hasValidSecurityConfig(): Boolean {
            return when (safetyMeasures) {
                1 -> serverSignKey.isNotBlank()
                2 -> serverPrivateKey.isNotBlank()
                3 -> serverSm4Key.trim().length == 32
                else -> false
            }
        }
        ...
        fun restoreSettings(cloneInfo: CloneInfo): Boolean {
            if (!App.HTTP_SERVER_FEATURE_ENABLED) return false
            return try {
                //保留设备名称、SIM卡主键/备注
